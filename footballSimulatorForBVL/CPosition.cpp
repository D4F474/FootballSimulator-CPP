...
CPosition::CPosition(std::string pos,float acceleration, float stamina, float strength, float sprintSpeed,
	...
	: POS(pos), ACCELERATION(acceleration), STAMINA(stamina), STRENGTH(strength), SPRINT_SPEED(sprintSpeed),
	DRIBBLING(dribbling), CROSSING(crossing), SHORT_PASS(shortPass),LONG_PASS(longPass),
	VISION(vision), SLIDE_TACKLE(slideTackle), STAND_TACKLE(standTackle), BALL_CONTROLL(ballControll),
	HEADING(heading), SHOT_POWER(shotPower),LONG_SHOT(longShot),PENALTIES(penalties),
	FINISHING(finishing), GK_POSSITIONING(GKPosstioning), GK_DIVING(GKDiving), GK_HANDLING(GKHandling),GK_KICKING(GKKicking), GK_REFLEXES(Reflexes)
{
